function Inventory(x,y,game){
    ...
    this.key_pick_up = game.add.audio('key_pickup',1,false);
    this.gated_door_soundeffect = game.add.audio('gated_door_open', 1, false);
    this.generator_stall = game.add.audio('generator_stall');
    this.generator_running = game.add.audio('generator_running');
    this.pour_gas = game.add.audio('pour_gas');
    this.gas_can_pickup = game.add.audio('gas_can_pickup');
    
    this.generatorIsFilled = false;
...
Inventory.prototype = {
    ...
    openDoor: function(door){
        ...
        else if(this.game.state.current == 'Level_One'){
            console.log('door1');
            if((this.doorList.getIndex(door) == 0 || this.doorList.getIndex(door) == 1) && this.contains('key1')){
                
                this.metal_door_open.play();
                this.doorList.getAt(0).barred = false;
                this.doorList.getAt(1).barred = false;
                door.visible = false;
                this.doorSetPieces.getAt(this.doorList.getIndex(door)).kill();
                
            }else if((this.doorList.getIndex(door) == 2 || this.doorList.getIndex(door) == 3) && this.contains('key2')){
                
                this.wooden_door_open.play();
                this.doorList.getAt(2).barred = false;
                this.doorList.getAt(3).barred = false;
                door.visible = false;
                this.doorSetPieces.getAt(this.doorList.getIndex(door)).kill();
                
            }else if(this.doorList.getIndex(door) == 4 && this.contains('key3')){
                     
                this.metal_door_open.play();
                this.doorList.getAt(4).barred = false;
                door.visible = false;
                this.doorSetPieces.getAt(this.doorList.getIndex(door)).kill();
                ...
                if(door.key == 'metal_door32' || door.key == 'metal_door' || door.key == 'metal_door_four')
                    this.metal_door_open.play();
                else
                    this.wooden_door_open.play();
                door.bar = false;
    ...
    specialInstruction: function(item){
        if(item.key == 'generator'){
            if(this.contains('jerry_can')){
                this.removeFromInventory('jerry_can')
                if(!this.pour_gas.isPlaying)
                    this.pour_gas.play();
                this.generatorIsFilled = true;
            }else{
                if(!this.pour_gas.isPlaying){
                    if(this.generatorIsFilled){
                        this.generatorIsRunning = true;
                        if(!this.generator_running.isPlaying)
                            this.generator_running.play();
                    }else
                        this.generator_stall.play();
                }
    ...
    removeWorldSetPiece: function(key){
        for(var i = 0; i < this.setPieceList.length; i++){
            if(this.setPieceList.getAt(i).key == key)
                this.setPieceList.getAt(i).destroy();
        }
    },
	...
	addToInventory: function(item){
        ...
        else if(item.key == 'key1'||item.key == 'key2'||item.key == 'key3')
            this.key_pick_up.play();
        else if(item.key == 'jerry_can')
            this.gas_can_pickup.play();
	},
